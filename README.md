...
-   기본적으로 master branch에 대한 규칙만 정확하게 정립되어 있다면 나머지 가지들에 대해서는 특별한 관여를 하지 않으며 pull request 기능을 사용하도록 권장하였습니다.
...
## 💡 느낀점
### 김동인
- 팀 프로젝트를 진행하면서 가장 크게 느낀 점은 프로젝트 초반에 요구사항, 화면 디자인, API, ERD 등의 프로젝트 전반적인 기초를 단단히 잡고 시작해야 한다는 것이었습니다. 이번 프로젝트는 기획이 충분히 되지 않은 상태로 기능 개발을 시작해서 프로젝트 중간에 요구사항을 다시 정리하며 시간을 많이 허비한 것 같습니다. 다음 프로젝트 때는 기획 단계에서 충분한 시간을 가지고 프로젝트 기초를 탄탄하게 다지고 개발에 들어가야 한다는 것을 배웠습니다.
- 코드를 수정할 때 복잡하고 가독성이 떨어지는 코드를 작성해서 불편함이 많았기 때문에 다음 프로젝트 때는 유지 보수성과 확장성이 좋은 코드를 쓸 수 있도록 더 고민하고 공부해야겠다고 느꼈습니다.
- 팀원들과 협업하는 과정에서 프로젝트 일정, 코드, 디자인의 디테일한 부분에 대해 더 많이 협의하고 소통하며 프로젝트를 진행했으면 좋았겠다는 아쉬움이 남았습니다. 그렇기 때문에 다음 프로젝트 때는 팀원들과의 소통에 더 많은 힘을 실어 이러한 부분들에 더 신경 써야겠다고 느꼈습니다.
---

### 김병규
### 좋았던 점

- 팀 프로젝트 기간 동안 팀원들 간의 협업에서 마찰이 생길 수도 있다고 생각했지만, 서로의 의견을 존중하고 화합하려는 노력을 통해 팀 분위기가 매우 좋았습니다.
- 프론트엔드와 백엔드의 구분 없이, 각자가 원하고 생각했던 기능들을 자유롭게 구현할 수 있었던 점이 인상적이었습니다.
- useQuery를 활용하여 실시간으로 데이터를 반영할 수 있었던 점이 특히 유익했습니다.
- MyBatis를 활용해 XML 기반으로 SQL 문법을 작성하여 원하는 데이터를 효율적으로 조회할 수 있었던 점이 좋았습니다.
- 프로젝트를 통해 각자의 역량을 확인하고 협업을 통해 시야를 넓힐 수 있었습니다. 또한 다양한 주제를 다루며 여러 배경지식을 공부해야 한다는 점을 많이 느꼈습니다.

### 아쉬웠던 점

- 팀원들이 각각 기능 위주로 프론트엔드와 백엔드를 작업하다 보니, 개인의 역량이 프로젝트의 성패에 크게 영향을 미쳤습니다. 이 과정에서 자신의 부족함이나 보완이 필요한 부분을 실감했던 점이 많았습니다.
- 프로젝트 초반에 문서 작업 및 데이터베이스 설계가 미흡했던 점이 아쉬웠습니다. 타 팀의 작업을 보며 불안감을 느껴 기초적인 DB 설계와 디자인을 명확히 정하지 않은 채 진행했고, 결과적으로 중후반에는 잦은 수정 작업으로 인해 시간이 더 소요되었습니다.
- 코드 컨벤션 및 Git 컨벤션이 처음 의도했던 대로 일관성 있게 적용되지 않았던 점도 아쉬웠습니다.
---
