...
### 김병규
- 팀 프로젝트를 각각 기능 위주로 프론트와 백엔드를 둘 다 각 팀원들이 작업을 하다보니 개인의 역량이 중요해졌고 자신의 부족함이나 필요했던 부분들이 작업을 하면서 몸으로 직접 와닿았던게 많았던거 같습니다. 그러다 보니 아쉬웠던 부분들이 더 많았습니다.
- 개발 간에는 기초 기반 틀을 다져가면서 타 팀을 보면서 내심 불안감에 휘둘려서 DB, 기초가 되는 디자인들을 확실하게 지정하고 진행을 했어야 했는데 분위기에 휩쓸려 마무리 못 짓고 작업을 시작해서 프로젝트 기간동안 게속 수정 및 반영을 프로젝트 작업을 하면서 해버려서 기초 기반 틀이 중요하다는 걸 느끼게 되었습니다. 추가적으로 DB를 먼저 전체 구축을 하고 관리자 페이지를 먼저 작업을 해서 받는 데이터, 뿌리는 데이터 , 보여지는 데이터들을 미리 선정했어야 헀는데 보이는 화면(사용자)을 우선적으로 작업을 하면서 미흡한 부분들이 생겼어서 작업의 속도가 좀 늦춰지기도 하고 관리자 페이지를 처음 구축을 하니까 어떤 부분들을 넣어야하고 관리자 입장에서 어떠한 작업을 할 수 있는지에 대한 부분들이 미흡했어서 프로젝트 작업에 어려움을 느끼게 되었습니다. 이번 프로젝트를 통해서 각자의 개개인의 역량이나 협업을 통해 보는 시야적인 측면도 좀 넓어진거 같고 다양한 주제를 통해 여러가지 배경지식들을 공부해야 한다는걸 많이 느꼈습니다.
---
